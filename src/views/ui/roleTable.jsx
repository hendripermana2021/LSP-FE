...
const TypeTables = () => {
  ...
  const handleSelectAll = (event) => {
    if (event.target.checked) {
      setSelectedRows(role.map((role) => role.id));
    } else {
      setSelectedRows([]);
    }
  };

  const handleSelectRow = (event, roleId) => {
    if (event.target.checked) {
  ...
  const deleteSelectedKelas = async () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await Promise.all(
            selectedRows.map(async (roleId) => {
              await axios.delete(`${serverDev}role/delete/${roleId}`, {
                headers: {
    ...
    <div>
      <Card>
        <CardTitle tag="h6" className="border-bottom p-3 mb-0">
          <i className="bi bi-border-all me-3"></i> Tabel Role
        </CardTitle>
        <CardBody>
          <Row>
            <Col>
          ...
          <br />
          <Table className="table table-hover" id="tablerole" responsive>
            <thead>
              <tr>
                <th>
                  <input
                    ...
                    checked={
                      selectedRows.length === role.length && role.length > 0
                    }
                  />
                </th>
                <th>No</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan="11">No Role available</td>
                </tr>
              ) : (
                role.map((roles, index) => (
                  <tr key={roles.id}>
                    <td>
                      <input
                        type="checkbox"
                        checked={selectedRows.includes(roles.id)}
                        onChange={(e) => handleSelectRow(e, roles.id)}
                      />
                    </td>
                    <td>{index + 1}</td>
                    <td>{roles.name_type}</td>
                    <td>
                      <DropdownButton
                        as={ButtonGroup}
                        key="end"
                        ...
                        title=""
                      >
                        <DetailTypeStuff typestuffprops={roles} />
                        <UpdateTypeStuff typestuffprops={roles} />
                      </DropdownButton>
                    </td>
                  </tr>
                ))

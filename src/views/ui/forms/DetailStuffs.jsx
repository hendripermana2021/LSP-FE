...
const DetailStuff = (props) => {
  const stuffprops = props.stuffprops;
  const [show, setShow] = useState(false);
  const [zoomed, setZoomed] = useState(false);
  const [namaBarang, setNamaBarang] = useState(stuffprops.name_stuff || "");
  const [deskripsi, setDeskripsi] = useState(stuffprops.deskripsi || "");
  const [price, setPrice] = useState(stuffprops.price || "");
  const [image, setImage] = useState(stuffprops.image || "");
  const [qty, setQty] = useState(stuffprops.qty || "");
  const [selectedTipeStuff, setSelectedTipeStuff] = useState(
    stuffprops.type.name_type || ""
  );
  const [diskon, setDiskon] = useState(stuffprops.disc || "");
  ...
  const handleUpdateBarang = async (e) => {
    ...
    try {
      const response = await axios.put(
        `${serverDev}stuff/update/${stuffprops.id}`,
        formData,
        {
          headers: {
                ...
                <Col lg={8}>
                  <Form.Group controlId="formImage">
                    ...
                    <div className="text-center">
                      <img
                        src={stuffprops.image || "/placeholder.png"}
                        alt="Barang"
...
DetailStuff.propTypes = {
  stuffprops: PropTypes.object,
};

export default DetailStuff;
